...
class LargeFileIDController(http.Controller):

    @http.route('/large_id/file/upload_id', type='http', auth="user", methods=['POST'], csrf=False)
    def restore_id(self, large_file):
        try:
            ...
            res = request.env.user.company_id.updateExternalID(save_path)
